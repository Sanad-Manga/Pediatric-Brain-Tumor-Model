...
The ensemble scores higher than both members on every region of this metric — errors of the two models are largely uncorrelated. **Caveat for ET:** 34 of the 82 held-out patients have no ET, and an empty region scores 1.0 when nothing is predicted (0.0 otherwise), so the ET figure partly rewards not hallucinating ET. On the 48 patients that do have ET the ensemble averages 0.492 vs 0.508 for the wide model alone (original 0.462); the ensemble's ET headline gain comes from fewer false ET predictions on ET-free patients (24/34 correct vs 18/34). NC/WT gains are real and broad. Per-patient scores for all three models are in `docs/per_patient_scores.csv`; regenerate with `overnight/per_patient_spread.py` and summarize with `overnight/analyze_spread.py`. Always report ET both ways in future comparisons. **Do not compare these numbers with the table above**: `evaluate.py` defines `NC = labels {1,2,3}`, whereas the table above uses `TC = {1,3}`, and the older figures were not measured with `--eval-plane both`.
